...
import VerifyPickupStats from '../components/verifyPickup/StatsCards';
// import PickupFilterPanel from '../components/pickup/FilterPanel';
import VerifyPickupFilter from '../components/verifyPickup/VerifyPickupFilter';
import { pickupApi, uploadFile, getFileUrl, depotsApi, companiesApi } from '../lib/api';
...
import { VerifyPickupDataTable } from '@/components/pickup/DataTableVerify';
...
export default function VerifyPickup() {
  ...
  const columns = [
    ...
    },
    {
      ...
      label: 'Weight Slip',
      render: (v, row) => {
        const fileId = row.weightment_slip_file_id;
        return fileId ? (
          <Button
            size="sm"
            variant="outline"
    ...
    //   label: 'Tare Slip',
    //   render: (v, row) => {
    //     const fileId = row.tare_slip_file_id || tareSlipFilesLocal[row.id];
    //     return fileId ? (
    //       <Button
    //         size="sm"
    //         variant="outline"
    ...
    // },
    {
      ...
      label: 'Weight (MT)',
      render: (v, row) => {
        const weight = row.loaded_weight_mt;
  ...
  const buildPickupParams = useCallback(() => {
    ...
    return params;
  }, [activeTab, filters.source_id, filters.source_type, filters.truck_number, filters.transporter_name, filters.driver_phone, filters.start_date, filters.end_date, tabs]);

  const pickupParams = useMemo(() => buildPickupParams(), [buildPickupParams]);

  const { data: allData = [], isLoading } = useQuery({
    queryKey: ['verify-pickups', pickupParams],
    queryFn: async () => {
      ...
      return rows.filter(row => row.status !== 'verified' && row.status !== 'final_verified');
    },
    enabled: canView,
    staleTime: 0,
  ...
  useEffect(() => {
    const loadDepots = async () => {
      try {
        const [depotsRes, companiesRes] = await Promise.all([
          depotsApi.getAll(),
          companiesApi.getAll()
        ]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (allData.some((d) => d.status === 'loading_started')) {
        setTick((prev) => prev + 1);
      }
    }, 1000);
  ...
  const handleWeightmentDone = async (row) => {
    if (!row) return;
    if (!canExecute) {
      return toast.error('You do not have permission to save weightment');
    }

    const loadedWeight = loadedWeights[row.id] || row.loaded_weight_mt;
    const slipFileId = weightmentSlipFiles[row.id] || row.weightment_slip_file_id;
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['verify-pickups'] });
    } catch (err) {
      ...
      <div className="flex gap-2 flex-nowrap items-center">
        ...
        {row.status === 'loaded' && (
          <div className="flex items-center gap-2 w-full">
            ...
            <Button
              size="sm"
              disabled={savingWeightment[row.id] || uploadingWeightment[row.id]}
              onClick={() => {
            ...
            >
              ...
              {savingWeightment[row.id] ? 'Submitting...' : 'Submit'}
            </Button>
          </div>
        )}

        {(row.status === 'loading_started' || row.status === 'scheduled') && row.tare_slip_file_id && (
          <div className="flex items-center gap-2">
            <input
              type="file"
              accept="image/*,.pdf"
  ...
  return (
    ...
    >
      ...
      />
      <VerifyPickupDataTable
        columns={columns}
        data={filteredData}
        loading={isLoading}
        customActions={customActions}
        emptyMessage={`No ${filters.status || 'loaded'} pickups found`}
      />
      ...
      <Dialog open={weightmentConfirmOpen} onOpenChange={setWeightmentConfirmOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-lg">Submit Confirmation</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 text-purple-800 text-sm">
              <div className="mb-3">
                <strong>Truck:</strong> {weightmentConfirmRow?.truck_number || '-'}
              </div>
              <div className="mb-3">
                <strong>Loaded Weight:</strong> {loadedWeights[weightmentConfirmRow?.id] || weightmentConfirmRow?.loaded_weight_mt || '-'} MT
              </div>
              <div className="mb-2">
              ...
              <div className="flex flex-col gap-1.5 text-sm">
                <div className="flex items-center gap-2">
                  <span className={`w-2 h-2 rounded-full ${weightmentConfirmRow?.tare_slip_file_id || tareSlipFilesLocal[weightmentConfirmRow?.id] ? 'bg-green-500' : 'bg-red-500'}`}></span>
                  <span>{weightmentConfirmRow?.tare_slip_file_id || tareSlipFilesLocal[weightmentConfirmRow?.id] ? 'Tare slip uploaded' : 'Tare slip not uploaded'}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`w-2 h-2 rounded-full ${weightmentSlipFiles[weightmentConfirmRow?.id] || weightmentConfirmRow?.weightment_slip_file_id ? 'bg-green-500' : 'bg-red-500'}`}></span>
                  <span>{weightmentSlipFiles[weightmentConfirmRow?.id] || weightmentConfirmRow?.weightment_slip_file_id ? 'Weight slip uploaded' : 'Weight slip not uploaded'}</span>
                </div>
              </div>
            </div>
            <SlideToConfirm
              label="Slide to Confirm Weightment Done →"
              onConfirm={submitWeightmentConfirmation}
            />
            <div className="flex justify-center">
              <Button
                variant="ghost"
